/**
 * Deploy HandMadeHub contract to a Midnight network.
 *
 * Non-interactive: scaffold → `npm run setup` runs straight through.
 * On public networks (preview / preprod) the wallet is generated locally,
 * printed, and the script polls for funding from the network faucet.
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║  Deploy HandMadeHub to ' + network);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      deployed = await deployContract(providers, {
        compiledContract: makeCompiledContract(contractModule, emptyWitnessValues()) as any,
        args: [],
